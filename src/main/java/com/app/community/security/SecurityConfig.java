...
public class SecurityConfig {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    ...
    http.formLogin(form -> form.loginPage("/login").loginProcessingUrl("/procssLogin").permitAll()
//        .successHandler(authenticationSuccessHandler)
        .failureHandler(authenticationFailureHandler)).logout(logout -> logout.logoutUrl("/logout") // 注销 URL
        .logoutSuccessHandler(logoutSuccessHandler).permitAll())// 允许所有用户注销
      .userDetailsService(userDetailsService).csrf(AbstractHttpConfigurer::disable);
